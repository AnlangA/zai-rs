...
async fn main() -> anyhow::Result<()> {
    ...
    let knowledge_id = std::env::args().nth(1).expect("Usage: knowledge_document_list <knowledge_id> [word]");
    let word_opt = std::env::args().nth(2);

    let mut q = DocumentListQuery::new(knowledge_id);
    if let Some(w) = word_opt { q = q.with_word(w); }
    ...
    if let Some(data) = &resp.data {
        println!("total={:?} list_len={}", data.total, data.list.as_ref().map(|v| v.len()).unwrap_or(0));
        if let Some(list) = &data.list {
            for (i, d) in list.iter().enumerate().take(5) {
                println!("#{} id={:?} name={:?} words={:?} bytes={:?} stat={:?}", i+1, d.id, d.name, d.word_num, d.length, d.embedding_stat);
            }
        }
    }
