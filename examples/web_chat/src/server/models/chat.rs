...
use zai_rs::model::{TextMessage, chat_models::GLM4_6, ThinkingType};
...
impl ChatCompletionBuilder {
    ...
    pub fn build(self) -> zai_rs::AppResult<zai_rs::model::ChatCompletion<GLM4_6, TextMessage>> {
        if self.messages.is_empty() {
            return Err(zai_rs::client::error_handler::ClientError::InvalidRequest(
                "No messages provided".to_string()
            ));
        }
        
        let mut client = zai_rs::model::ChatCompletion::new(
            self.model,
...
pub mod chat_utils {
    ...
    pub fn extract_text_from_response(response: &zai_rs::model::ChatBaseResponse) -> Option<String> {
        response
            .choices()
    ...
    pub fn extract_reasoning_from_chunk(chunk: &zai_rs::model::ChatStreamResponse) -> Option<String> {
        chunk
            .choices
            .get(0)?
            .delta
...
- Keep responses relevant to the user's question"#.to_string()
    }
}
...
mod tests {
    ...
    fn test_chat_utils() {
        ...
        assert_eq!(assistant_msg.content, serde_json::json!("Assistant message"));
    }
}