...
use crate::server::models::chat_utils;
    ...
    pub fn update_with_request(
        ...
        if let Some(avg) = self.avg_response_time_ms {
            self.avg_response_time_ms =
                Some((avg * (self.request_count - 1) + response_time_ms) / self.request_count);
        } else {
    ...
    pub fn create(
        ...
        let content = match format {
            ...
            ExportFormat::Markdown => {
                export_as_markdown(session, include_metadata, include_timestamps)
            },
...
fn export_as_markdown(
    ...
    if include_metadata {
        content.push_str(&format!(
            "**Think Mode:** {}\n",
            if session.metadata.think_mode {
                "Enabled"
            } else {
                "Disabled"
            }
    ...
    for message in &session.messages {
        let role = chat_utils::text_message_role(message);
        ...
        content.push_str(&chat_utils::text_message_content(message));
...
fn export_as_html(
    ...
    if include_metadata {
        ...
        content.push_str(&format!(
            "<p><strong>Request Count:</strong> {}</p>\n",
            session.metadata.request_count
        ));
    }
    ...
    for message in &session.messages {
        ...
        let content_str = chat_utils::text_message_content(message);
        content.push_str(&format!(
            "<p>{}</p>\n",
