...
impl SessionStore {
    ...
    pub async fn cleanup_expired(&self) {
        ...
        let expired_keys: Vec<String> = self.sessions
            ...
            .filter(|entry| {
                ...
                now.signed_duration_since(session.last_activity).num_seconds() > self.timeout as i64
            })
    ...
    pub fn stats(&self) -> SessionStats {
        SessionStats {
            ...
            activeSessions: self.sessions
                .iter()
                .filter(|entry| {
                    let session = entry.value();
                    Utc::now().signed_duration_since(session.last_activity).num_seconds() < 300 // 5 minutes
                })
                .count(),
        }
    }
...
impl RateLimiter {
    ...
    pub fn is_allowed(&self, ip: &str, limit: u64, window_seconds: u64) -> AppResult<bool> {
        let now = Utc::now();
        let mut entry = self.requests.entry(ip.to_string()).or_insert_with(|| RateLimitData {
            count: 0,
            window_start: now,
            total_requests: 0,
        });
    ...
    pub fn stats(&self) -> RateLimitStats {
        ...
        let total_requests: u64 = self.requests
            .iter()
