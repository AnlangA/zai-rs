...
async fn main() -> Result<(), Box< dyn std::error::Error>> {
    ...
    println!("✅ Parallel results: {} successful calls", parallel_results.len());
    
    // Test 4: Schema caching (execute same tool multiple times)
    let start = Instant::now();
    for _ in 0..20 {
        let _result = executor.execute_simple(
            "test_tool_0",
            json!({ "input": "schema caching test" })
        ).await?;
    }
    let cached_time = start.elapsed();
    println!("✅ Executed same tool 20 times (with schema caching) in {:?}", cached_time);
    ...
    let retry_executor = ToolExecutor::builder()
        .retries(3)
        .build();
    ...
    let start = Instant::now();
    let retry_result = retry_executor.execute_simple("failing_tool", json!({})).await;
    let retry_time = start.elapsed();
    
    match retry_result {
        Ok(_) => println!("❌ Unexpected success on failing tool"),
        Err(e) => println!("✅ Expected error after retries: {} (took {:?})", e, retry_time),
    }
