...
impl KnowledgeListRequest {
    ...
    fn rebuild_url(&mut self, q: &KnowledgeListQuery) {
        let mut url = Url::parse("https://open.bigmodel.cn/api/llm-application/open/knowledge").unwrap();
        {
    ...
    pub async fn send_with_query(mut self, q: &KnowledgeListQuery) -> anyhow::Result<KnowledgeListResponse> {
